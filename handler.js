...
╔═════════════════════╗
 ❰ ⚠️ ❱ *¡𝗨𝗦𝗘𝗥 𝗕𝗔𝗡𝗡𝗘𝗗!* ❰ ⚠️ ❱
—◉ *Notice ${messageNumber}/3 (Total: 3)* —◉ ${user.bannedReason ? `\n*Reason:* ${user.bannedReason}` : '*Reason:* Unspecified'} —◉ *If you think this is a mistake and you have proof, you can contact the Bot owner to appeal the suspension.* —◉ *Contact owner for appeals:* 
╚═════════════════════╝
               `.trim();
              m.reply(messageText);
        ...
        if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
          mconn.conn.reply(m.chat, `*[ 💎 ] Your diamonds (limits) have been exhausted, you can purchase more with the command ${usedPrefix}buyall*`, m);
          continue; 
        }
        if (plugin.level > _user.level) {
          mconn.conn.reply(m.chat, `*[ 💠 ] You are required to have the level ${plugin.level} to be able to use this command. Your current level is ${_user.level}, use the command ${usedPrefix}lvl to raise your level with exp.*`, m);
          continue; 
        }
        const extra = {
          match,
        ...
        try {
          ...
          if (m.limit) {
            m.reply('*[ 💎 ] ' + +m.limit + ' diamond(s) (limits).*');
          }
        }
        break;
      }
...
export async function callUpdate(callUpdate) {
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        const callmsg = await mconn.conn.reply(nk.from, `Hello *@${nk.from.split('@')[0]}*, las ${nk.isVideo ? 'videollamadas' : 'llamadas'} are not allowed, you will be blocked.\n-\nIf you accidentally called contact my creator to unblock you!`, false, {mentions: [nk.from]});
...
┏━━━━━━━━━⬣  𝘼𝙉𝙏𝙄 𝘿𝙀𝙇𝙀𝙏𝙀  ⬣━━━━━━━━━
*■ User:* @${participant.split`@`[0]} *■ Time:* ${time} *■ Date:* ${date} *■ Sending the deleted message...* *■ To disable this feature, type the command:* *—◉ #disable antidelete*
┗━━━━━━━━━⬣  𝘼𝙉𝙏𝙄 𝘿𝙀𝙇𝙀𝙏𝙀  ⬣━━━━━━━━━`.trim();
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] 👑 This command can only be used by the *Creator of the bot*',
        owner: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] 👑 This command can only be used by the *Creator of the bot*',
        mods: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] 🔰 This feature is only for *For Bot Moderators*',
        premium: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] 💠 This command is only for *Premium* members*',
        group: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] ⚙️ This command can only be used in groups!',
